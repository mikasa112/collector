...
def translate_xlsx(input_file, output_file):
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        for sheet_name, df in all_sheets.items():
            ...
            if "点位名称" not in df.columns:
                df.to_excel(writer, sheet_name=sheet_name, index=False)
                continue

            source_texts = [
                str(value) if pd.notnull(value) else "" for value in df["点位名称"]
            ]
            ...
            if "键" in df.columns:
                ...
                df.insert(loc=len(df.columns), column="键", value=translated_key_series)
...
if __name__ == "__main__":
    input_xlsx = "./config/永泰_2_BCU.xlsx"
    output_xlsx = "./config/永泰_2_BCU_trans.xlsx"
